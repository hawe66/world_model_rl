...
- Task 4 진행 중(로컬 DoD 충족, 클라우드 본검증 대기): `wm/algos/rssm/`(modules·rssm) PlaNet RSSM + `wm/configs/rssm/default.yaml` + `scripts/train_rssm.py`(--resume·--smoke) + `tests/test_rssm.py`(신규 6개 green). GRU deter + Gaussian stoch, decoder/reward는 `[h,z]` 결합 state 입력. 손실=recon NLL+reward NLL+**latent overshooting**(Full: 각 t·d∈[1,D] posterior@t→prior d-step roll, `KL(sg(q[t+d])‖prior_d)`, 1/D 평균, free_nats=3 clamp, is_first 경계 중단; **사용자 확인**) — KL balancing은 미도입(의도적, Task 6 복선). **버퍼 리셋 계약**: observe()가 t=0 무조건 init + is_first 추가 마스킹 리셋(단위 테스트됨). hparam은 PlaNet recall값(부록 확인 시 정정). smoke(pong, CPU): loss 438→365 단조↓·KL 비붕괴·NaN 없음·ckpt/open-loop 저장 확인. **클라우드 GPU walker-walk vision에서 DoD 4~6(open-loop 20step↑ 유지·reward MSE↓·KL 비붕괴) 본검증 필요**. 상세 `docs/notes/rssm.md`.
- Task 3 완료: `wm/common/`에 buffer·logger·eval·viz 공통 인프라 + `scripts/demo_buffer_viz.py` + `tests/test_buffer.py`·`tests/test_infra.py`(신규 15개 green). `SequenceReplayBuffer`(시퀀스 스트림 샘플링, **경계 허용+is_first 리셋=DreamerV2/V3 방식, 사용자 확인**; 링 FIFO; uint8 저장+to_float; `state_dict`로 checkpoint `--resume` 연동; `from_npz`). `Logger`(TensorBoard+metrics.jsonl, env step 축). `evaluate(env,policy,...)`(eval env 분리 계약, 랜덤 policy 검증). viz(`frames_to_grid`/`frames_to_gif`/`open_loop_grid`, 모델 부재라 pred=real). 데모로 pong.npz→`(4,16,64,64,1)` 배치+grid/gif 저장 확인. 함정: imageio GIF `fps`→`duration` deprecated 교체. config yaml 배선은 Task 4 train으로 이월. 상세 `docs/notes/infra.md`.
- Task 2 완료: `wm/common/envs/`(base·atari·dmc) 공통 `Transition` 인터페이스 + `scripts/collect_random.py` + `tests/test_envs.py`. Atari(ALE, sticky OFF, action repeat 4, max-pool 2프레임, grayscale/RGB, life-loss 옵션) · DMC(proprio/vision, action repeat 2, walker-walk·cheetah-run) 구현. 결정성 테스트 포함 신규 14개(vision 1개는 로컬 GL 부재로 skip). Pong·walker-walk npz 수집 확인. **경계 플래그 결정(사용자 확인)**: `is_last`=terminated∨truncated, `is_terminal`=terminated만(truncation 제외), DMC는 is_terminal 항상 False, Atari life-loss는 기본 OFF·켜면 목숨감소→is_terminal. 이미지는 HWC uint8 저장. **의존성**: dm_control 1.0.41 ↔ mujoco 3.9 비호환(flex_bandwidth) → `mujoco>=3.8.1,<3.9` 핀 고정(사용자 확인). DMC vision 렌더링은 이 머신 GL 부재로 로컬 불가 → proprio 로컬 검증, vision은 클라우드. 함정 상세는 `docs/notes/envs.md`. config yaml은 소비자(train) 부재로 Task 4+로 이월.
